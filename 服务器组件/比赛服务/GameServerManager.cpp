...
//停止服务
bool  CGameServiceManager::StopService()
{
...
//启动服务
bool  CGameServiceManager::StartService()
{
	//启动时间
...
#ifdef _DEBUG
	m_pITimerEngine->SetTimer(IDI_HALF_MINUTE,1000,TIMES_INFINITY,0);
#else
...
//初始化接口
bool  CGameServiceManager::InitMatchInterface(ITCPNetworkEngineEvent *pTCPNetworkEngine,IDataBaseEngine *pIDataBaseEngine, IServerUserManager *pIServerUserManager,
...
//用户参加比赛
bool  CGameServiceManager::OnUserJoinGame(IServerUserItem * pIServerUserItem, BYTE cbReason)
{
	...
	for (INT_PTR i=0;i<nCount;i++)
	{
		...
		if(pUserItem==pIServerUserItem && pUserItem->GetTableID()!=INVALID_TABLE)
		{
...
//用户退出比赛
bool  CGameServiceManager::OnUserQuitGame(IServerUserItem * pIServerUserItem, BYTE cbReason,WORD *pBestRank, DWORD dwContextID)
{	
	ASSERT(pIServerUserItem!=NULL);
	if (pIServerUserItem == NULL) return false;
...
//时间事件
bool  CGameServiceManager::OnEventTimer(DWORD dwTimerID, WPARAM dwBindParameter)
{
	...
	case IDI_DELETE_OUT_USER:
		{
			...
			for (INT_PTR i=0;i<nCount;i++)
			{
				...
				if(pUserItem!=NULL)
				{
					//OnUserQuitGame(pUserItem,0);
					if(pUserItem->GetTableID()!=INVALID_TABLE)
					{
						//pUserItem->SetUserTeamID(INVALID_TABLE);
						//pUserItem->SetUserSort(0,EN_NULL);
 						m_ppITableFrame[pUserItem->GetTableID()]->PerformStandUpAction(pUserItem);
// 						if(pUserItem->IsAndroidUser())
						//m_pGameMatchSink->m_pIGameServiceFrame->DeleteUserItem(pUserItem);	
						
					}
					
//  					if(pUserItem->IsActive())
//  						m_pGameMatchSink->m_pIGameServiceFrame->DeleteUserItem(pUserItem);
	...
	case IDI_DELETE_OVER_MATCH:
		{
					...
					if(pTempMatch==pOverMatch)
					{
						for (WORD j=0;j<pOverMatch->m_wTableCount;j++)
						{
							...
							for(WORD k=0; k<CChampionshipMatch::m_wChairCount; k++)
							{
								...
								if(pUserTemp!=NULL && !pOverMatch->m_pAssignTable[j].pTableFrame->PerformStandUpAction(pUserTemp))
								{
									CString str;
			...
			while(m_UnRecovery.GetCount()>0)
			{
				...
				for (WORD i=0; i<CChampionshipMatch::m_wChairCount;i++)
				{
					...
					if(pUserItem!=NULL && !m_ppITableFrame[wTableID]->PerformStandUpAction(pUserItem))
					{
						bSuccess=false;
						break;
					}
...
//参数修改
bool  CGameServiceManager::RectifyServiceOption(tagGameServiceOption * pGameServiceOption,tagGameServiceAttrib *pGameServiceAttrib)
{
	//效验参数
...
//建立比赛模式
void *  CGameServiceManager::CreateGameMatchSink(const IID & Guid, DWORD dwQueryVer)
{
	//变量定义
...
//发送数据
bool  CGameServiceManager::SendDataToServer(DWORD dwUserIndex, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	//构造数据
	TCP_Command Command;
...
//			m_pGameMatchSink->m_pIGameServiceFrame->SendAndroidDataToServer(dwUserIndex,wMainCmdID,wSubCmdID,pData,wDataSize);
		{
...
//报名人满，开始比赛
bool CGameServiceManager::OnMatchStart(CChampionshipMatch *pMatch)
{
	try
	{
...
//等待超时
bool CGameServiceManager::OnWaitTimeOut(DWORD dwMatchNO)
{
	return true;
}

//用户淘汰
bool CGameServiceManager::OnUserOut(IServerUserItem *pUserItem,LPCTSTR szReason,WORD wLen,WORD wRank,CChampionshipMatch *pMatch)
{
	if(pUserItem==NULL)
		return false;
	...
	if(wLen>0)
	{
		...
		SendGameMessage(pUserItem,str,SMT_EJECT|SMT_TABLE_ROLL|SMT_CLOSE_GAME);
	}
	...
	if(m_pIDataBaseEngine!=NULL && wRank >=m_RewardArr.GetCount())
	{
...
//删除用户
bool CGameServiceManager::DeleteUserItem(DWORD dwUserIndex)
{
	try
	{
		if (m_pITCPNetworkEngineEvent == NULL)
...
//发送消息
bool CGameServiceManager::SendRoomMessage(IServerUserItem * pIServerUserItem, LPCTSTR lpszMessage, WORD wMessageType)
{
	if(pIServerUserItem==NULL /*|| !pIServerUserItem->IsActive()*/)
		return false;
	//构造数据包
	CMD_CM_SystemMessage Message;
...
//发送游戏消息
bool CGameServiceManager::SendGameMessage(IServerUserItem * pIServerUserItem, LPCTSTR lpszMessage, WORD wMessageType)
{
...
//初始化桌子框架
bool CGameServiceManager::InitTableFrame(ITableFrame * pTableFrame,WORD wChairID)
{
...
//分配桌子
bool CGameServiceManager::AssignTable(CChampionshipMatch *pMatch, WORD wTableCount)
{
	...
	while((wCount<wTableCount)&&n<m_pGameServiceOption->wTableCount)
	{
		if(!m_bTableUsed[n])
		{
			...
			{
				if(m_ppITableFrame[n]->GetTableUserItem(i)!=NULL)
				{
					CTraceService::TraceString(TEXT("严重错误，分配的桌子不是空桌子。"),TraceLevel_Exception);
					bEmpty=false;
					wCount++;
					break;
				}
...
//发送数据
bool CGameServiceManager::SendData(IServerUserItem * pIServerUserItem, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
...
//读取比赛规则
bool CGameServiceManager::ReadMatchOption(LPCTSTR lpFileName)
{
...
//定时器
bool CGameServiceManager::SetGameTimer(DWORD dwTimerID, DWORD dwElapse, DWORD dwRepeat, WPARAM dwBindParameter,CChampionshipMatch *pMatch)
{
	tagMatchTimerItem * pTimerItem=NULL;
...
// bool CGameServiceManager::SendGameMessage(IServerUserItem * pIServerUserItem, LPCTSTR lpszMessage, WORD wMessageType)
// {
// 	return SendGameMessage(pIServerUserItem,lpszMessage,wMessageType);
// }

//写入奖励
bool CGameServiceManager::WriteUserAward(IServerUserItem *pIServerUserItem, WORD wRank,DWORD dwMatchNO,LPCTSTR szDescribe,CChampionshipMatch *pMatch)
{
	//参数效验
	ASSERT(pIServerUserItem!=NULL);
	if(pIServerUserItem==NULL) return false; 
	...
	else
	{
		if(szDescribe==NULL)
		{
			...
			str.Format(TEXT("比赛已结束，恭喜您获得%d名。谢谢您的参与。您可以安全关闭比赛客户端，继续报名参加其他比赛。"),
				wRank+1);
			SendGameMessage(pIServerUserItem, str,SMT_CHAT|SMT_TABLE_ROLL);
		}
		else
		{
			SendGameMessage(pIServerUserItem, szDescribe, SMT_CHAT|SMT_TABLE_ROLL);
		}
	}
	return true;
}
...
//比赛结束
bool CGameServiceManager::OnEventMatchOver(CChampionshipMatch *pMatch)
{
...
//发送用户分数
bool CGameServiceManager::SendGroupUserScore(IServerUserItem * pIServerUserItem,CChampionshipMatch *pMatch)
{
...
//发送状态
bool  CGameServiceManager::SendGroupUserStatus(IServerUserItem * pIServerUserItem,CChampionshipMatch *pMatch)
{
	//效验参数
	ASSERT(pIServerUserItem!=NULL);
	...
	//UserStatus.UserStatus.wTeamID=pUserData->wTeamID;
...
//用户比赛成绩
bool CGameServiceManager::OnUserMatchResult(DWORD dwUserID, WORD wKindID, WORD wServerID, WORD wSortID)
{
	return true;
}

//获取比赛信息
// bool CGameServiceManager::OnGetGameMatchInfo(CMD_GR_GameMatchInfo &MatchInfo)
// {
// 	return true;
// }

//排序用户
void CGameServiceManager::UserSort(WORD wTeamID)
{

}

//发送数据到服务器
bool CGameServiceManager::SendDataToGameServer(DWORD dwAndroidID, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	return SendDataToServer(dwAndroidID,wMainCmdID,wSubCmdID,pData,wDataSize);
}

//发送数据到一组用户
bool CGameServiceManager::SendGroupData(WORD wMainCmdID, WORD wSubCmdID, void * pData, WORD wDataSize,CChampionshipMatch *pMatch)
{
	POSITION pos=pMatch->m_OnMatchUserMap.GetStartPosition();
	IServerUserItem *pUserItem=NULL;
	DWORD dwUserID=0;
...
//获取一个空闲的机器人
IAndroidUserItem * CGameServiceManager::GetFreeAndroidUserItem()
{
...
//发送数据
bool CGameServiceManager::SendMatchInfo(IServerUserItem * pIServerUserItem)
{
	CMD_GR_Match_Num MatchNum;
...
//数据库事件
bool CGameServiceManager::OnEventDataBase(WORD wRequestID, IServerUserItem * pIServerUserItem, VOID * pData, WORD wDataSize)
{
	switch (wRequestID)
	{
...
//命令消息
bool CGameServiceManager::OnEventSocketMatch(WORD wSubCmdID, VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem, DWORD dwSocketID)
{
	switch (wSubCmdID)
	{
...
//	return g_GameServiceManager.QueryInterface(Guid,dwInterfaceVer);
	//建立对象
	CGameServiceManager * pGameService=NULL;
	try
	{
		pGameService=&g_GameServiceManager;
