...
VOID CTableViewFrame::OnPaint()
{
	...
	if((m_wServerType&GAME_GENRE_MATCH)!=0)
	{
		...
		if(m_pMatchDesc!=NULL)
		{
			for(BYTE i=0; i<MAX_MATCH_DESC; i++)
			{
				if(m_pMatchDesc->szTitle[i][0]!='\0')
				{
					pBufferDC->SetTextColor(m_pMatchDesc->crTitleColor);
					pBufferDC->DrawText(m_pMatchDesc->szTitle[i],&CRect(10,15+i*30,80,35+i*30),DT_SINGLELINE|DT_LEFT|DT_END_ELLIPSIS);
					pBufferDC->SetTextColor(m_pMatchDesc->crDescribeColor);
					pBufferDC->DrawText(m_pMatchDesc->szDescribe[i],&CRect(80,15+i*30,280,35+i*30),DT_SINGLELINE|DT_LEFT|DT_END_ELLIPSIS);
				}
				else
					break;
			}
