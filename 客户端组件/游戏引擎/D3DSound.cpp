...
bool CD3DSound::CreateD3DSound(HWND hWnd)
{
	...
	TraceService->TraceString(TraceLevel_Normal,TEXT("Sound Device: %s"),CA2CT(_DeviceInfo.name));
	TraceService->TraceString(TraceLevel_Normal,TEXT("Sound Device Driver: %s"),CA2CT(_DeviceInfo.driver));
...
{
	m_hStream = LoadSound(pszFileName, bRepetition);
	if (m_hStream != 0)
	{
		PlaySound(m_hStream);
		return true;
...
{
	m_hStream = LoadSound(hInstance, pszResource, pszTypeName, bRepetition);
	if (m_hStream!=0)
	{
		PlaySound(m_hStream);
		return true;
